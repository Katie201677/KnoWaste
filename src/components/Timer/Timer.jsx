...
export const setDeadline = (days, hours) => {
  let deadlineThisWeek = DateTime.local().startOf('week').plus({days: days, hour: hours }) //Friday at 6pm
  const deadline=  (now.weekday == deadlineThisWeek.weekday && now.hour > deadlineThisWeek.hour) ? deadlineThisWeek.plus({days: days + 7}) : deadlineThisWeek; 
  return deadline;
}
//Time is the parameter we need to pass to mock for testing.
  ...
  const diff = upcomingDeadline.diff(time, ["days", "hours", "minutes", "seconds", "milliseconds"])
  return diff.values
}  

export const lessThanSixHours = (days, hours) => (days == 0 && hours < 6) ? true : false;
//The Timer component
...
const Timer = () => {
  ...
  const getTimeDifference = () => {
    let upcomingDeadline = setDeadline(days, hours)
    const diff = upcomingDeadline.diff(now,["days", "hours", "minutes", "seconds", "milliseconds"])
    return diff.values
  }
 const timerStyling = (d,h,m,s) => {
  ...
  return (
      ...
      <div className={`${styles.timer} textwhite box-style-1 `}>
        { isTimerRunout? null
              : <h2 className={isRed ? "textred" : "textwhite"}>{timeLeftStr} until orders close on Friday at 6pm</h2>
          }
